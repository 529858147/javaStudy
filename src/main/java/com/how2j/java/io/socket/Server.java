...
public class Server {
    ...
    public static void initServer() {
        try {
            ...
            System.out.println("启动服务器：" + getDate());
            Socket socket = serverSocket.accept();
            System.out.println("客户端: " + socket.getInetAddress() + ":" + socket.getLocalPort() + ",连接服务器成功: " + getDate());
            //构建服务器的IO
            ...
            while (true) {
                if ((len = bufferedReader.read(chars)) != -1) {
                    ...
                    if (str.indexOf("\n") > -1) {
                        System.out.print(getDate() + ",服务器接收数据：" + stringBuilder);
